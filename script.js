...
const account1 = {
  ...
  movementsDates: [
    ...
    '2025-02-20T10:17:24.185Z',
    '2025-02-24T14:11:59.604Z',
    '2025-02-25T17:01:17.194Z',
    '2025-02-26T23:36:17.929Z',
    '2025-02-27T10:51:36.790Z',
  ],
  currency: 'EUR',
  locale: 'pt-PT', // de-DE
};
...
let now = new Date();
labelDate.textContent = new Intl.DateTimeFormat('hr-HR').format(now);

const calcDayPassed = (date1, date2) =>
  Math.round(Math.abs(date2 - date1) / (1000 * 60 * 60 * 24));

const displayDate = (currentDate, movementDate) => {
  let daysPassed = calcDayPassed(currentDate, movementDate);

  if (daysPassed === 0) return 'Today';
  if (daysPassed === 1) return 'Yesterday';
  if (daysPassed <= 7) return `${daysPassed} days ago`;
  else return movementDate.toLocaleDateString();
};
...
const displayMovements = function (acc, sort = false) {
  ...
  //SORTING BEFORE COMBINEDMOVESDATA OBJECT
  // const movs = sort
  //   ? acc.movements.slice().sort((a, b) => a - b)
  //   : acc.movements;

  combinedMovesDates.forEach(function (object, i) {
    const { movement, movementDate } = object;

    const type = movement > 0 ? 'deposit' : 'withdrawal';
    let date = new Date(movementDate);

    let daysPassed = displayDate(new Date(), date);

    let formattedMovement = formatCurrency(Math.abs(movement));
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    labelDate.textContent = `${currentDay} ${currentDate} ${currentMonth}, ${currentYear}`;
...
// console.log(Number.parseInt('€30'));
// console.log(Number.parseFloat('300€'));
